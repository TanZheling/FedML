...
from .datasets import CRCK
from .datasets import CRCK_truncated
...
def get_dataloader_CRCK_truncated(
    CRCK_dataset_train,
    CRCK_dataset_test,
    train_bs,
    test_bs,
    ...
    # dl_obj = CRCK
    transform_train, transform_test = _data_transforms_CRCK()
    ...
    #     download=False,
    # )
    train_ds = CRCK_dataset_train
    test_ds = CRCK_dataset_test
...
def load_partition_data_CRCK(
    ...
    transform_train, transform_test = _data_transforms_CRCK()
    train_dataset = CRCK(root=trainset, ann_file=train_anno, cls_ind=task, train=True, transform=transform_train)
    test_dataset = CRCK(root=testset, ann_file=test_anno, cls_ind=task, train=False,transform = transform_test)
    ...
    train_data_global, test_data_global = get_dataloader_CRCK_truncated(
        train_dataset,
        test_dataset,
        train_bs=batch_size,
        test_bs=batch_size,
    ...
    for client_idx in range(client_number):
        ...
        #                                          dataidxs)
        client_anno_this = client_anno+'/train_'+str(client_idx)+'.txt'
        client_trainset = CRCK(root=trainset, ann_file=client_anno_this, cls_ind=task, train=True, transform=transform_train, noise_num=noise_num)
        client_testset = test_dataset
        train_data_local, test_data_local = get_dataloader_CRCK_truncated(
            client_trainset,
            client_testset,
            train_bs=batch_size,
            test_bs=batch_size
        )
        data_local_num_dict[client_idx] = len(client_trainset)
...
if __name__ == "__main__":
    ...
    client_data_dir = "/shared/dqwang/datasets/CRC/annotation/federate/split_5_1"
    all_anno = "/shared/dqwang/datasets/CRC/annotation/train_ann.txt"
